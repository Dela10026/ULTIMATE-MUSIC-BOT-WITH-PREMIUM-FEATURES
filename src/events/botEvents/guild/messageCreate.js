...
module.exports.run = async (client, message) => {
  ...
  const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setLabel("Vote Bot").setURL(voteUrl).setStyle(ButtonStyle.Link));
  ...
  if (command.owner && message.author.id !== client.owner) {
    return message.reply({ content: `ℹ️| Only my owner can use this command!\n\n© Bot 2023` });
  }
  ...
  try {
    ...
    return message.reply({ content: `ℹ️| Something went wrong.`, components: [row] });
  }
};
