...
module.exports.run = async (client, interaction) => {
  if (interaction.type === InteractionType.ApplicationCommand) {
    ...
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setLabel("Vote Bot").setURL(voteUrl).setStyle(ButtonStyle.Link),
    );
    ...
    if (userBan && userBan.isBanned === true && interaction.user.id !== client.owner) {
      return interaction.reply({
        content: `ℹ️ | You are banned from using ${client.user}, click the button support to appeal.`,
        components: [row],
        ephemeral: true,
      });
    }
    ...
    if (botMissingPermissions.length > 0) {
      return interaction.reply({
        content: `ℹ️ | I don't have one of these permissions \`ViewChannel\`, \`SendMessages\`, \`EmbedLinks\`.\nPlease double check them in your server role & channel settings.`,
        components: [row],
        ephemeral: true,
      });
    }
    ...
    if (!interaction.guild.members.cache.get(client.user.id).permissions.has(command.permissions.bot || [])) {
      warning.setDescription(`ℹ️ | I don't have permission \`${command.permissions.bot.join(", ")}\` to execute this command.`);
    ...
    if (!interaction.member.permissions.has(command.permissions.user || [])) {
      warning.setDescription(
        `ℹ️ | You don't have permission \`${command.permissions.user.join(", ")}\` to execute this command.`,
      );
    ...
    if (command.settings.player && !player) {
      warning.setDescription(`ℹ️ | There isn't player exists for this server.`);
    ...
    if (command.settings.current && !player.currentTrack) {
      warning.setDescription(`ℹ️ | There isn't any current playing right now.`);
    ...
    if (command.settings.inVc) {
      if (!channel) {
        warning.setDescription(`ℹ️ | You must be in a voice channel to use this command.`);
      ...
      if (
        ...
        warning.setDescription(
          `ℹ️ | I don't have permission \`${command.permissions.channel.join(
            ", ",
    ...
    if (command.settings.sameVc) {
      if (!channel || interaction.member.voice.channelId !== interaction.guild.members.me.voice.channelId) {
        warning.setDescription(`ℹ️ | You must be on the same voice channel as mine to use this command.`);
    ...
    if (command.settings.premium) {
      if (user && !user.isPremium) {
        warning.setDescription(`ℹ️ | You're not premium user!`);
    ...
    if (command.settings.owner && interaction.user.id !== client.owner) {
      warning.setDescription(`ℹ️ | Only my owner can use this command!\n\n© Fancy 2023`);
    ...
    try {
      ...
      warning.setDescription(`ℹ️ | Something went wrong.`);
